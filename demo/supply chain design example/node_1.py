...
class node_1(): 
    ...
    def decision_model(self):
        ...
        depot_flow = m.addConstrs((gp.quicksum(flow_dc.select(depot, '*')) == gp.quicksum(flow_fd.select('*', depot))
                               for depot in depots), name="depot")
        
        # Depot throughput
        depot_capacity = m.addConstrs((gp.quicksum(flow_fd.select('*', depot)) <= through[depot]
        ...
        coupling_send_list = np.array([np.array([flow_fd_list],dtype=object),
                                  np.array([],dtype=object),
                                  np.array([flow_dc_list],dtype=object)],
                                 dtype=object)
        coupling_receive_list = np.array([np.array([flow_fd_list],dtype=object),
                                  np.array([],dtype=object),
                                  np.array([flow_dc_list],dtype=object)],
                                 dtype=object)
        ...
        transport_cost = (gp.quicksum(flow_dc[arc] * cost_dc[arc] for arc in arcs_dc) )
