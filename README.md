# General-Augmented-Lagrangian-Coordination(GeneralALC)
## Introduction
...
This project proposes a graph-based general ALC(GeneralALC) method that can model networks or problems of arbitrary network structure, but is only applicable to bidirectional symmetric coordination.
...
1. Simple operation, only need to input the adjacency matrix and the decision model of each decision node to realize, no need to realize the coordination principle
2. Arbitrary coordination order, this project can realize seamless switching of arbitrary coordination order
3. Unified modeling and solving framework  
## Principle
...
1. Write the adjacency matrix based on the coordination relationship graph, where coupling_send_list_i of decision node i is the ith row of the adjacency matrix and coupling_receive_list_i is the ith column of the adjacency matrix  
![协调关系的邻接矩阵](./ALC_adjacency_matrix.png)
1. Write the above coupling_send_list_i and coupling_receive_list_i according to the template corresponding to node_i.py  
2. Fine tune the ALC_coordination.py file and run it! 
## Application Notes
...
## 引言
Augmented Lagrangian Coordination(ALC) 是一种决策节点之间的协调方法，其中每个决策节点/单元拥有自己的决策能力，能够根据自我目标和约束(条件)做出决策，而节点之间的决策需要进行相互协调以达成一致和优化整个网络的性能。  
该项目提出了一种基于图的通用ALC(GeneralALC)方法，能够对任意网络结构的网络或问题进行建模，但只适用于双向对称协调；
...
此外，本文还在demo中增加了关于供应网络设计的案例。
### 项目应用步骤
1. 画出节点间的协调关系图以及其中耦合的变量(交互的决策信息)，以及每个节点的目标和约束，其中目标额外包含一个不一致性惩罚，其中不一致性惩罚的输入是一个列表(coupling_receive_list), 该列表由下一步骤给出  
![节点间的协调关系图](./graph_of_coordination_relationship.png)
1. 根据协调关系图写出邻接矩阵，其中决策节点i的coupling_send_list_i就是邻接矩阵第i行，coupling_receive_list_i是邻接矩阵第i列  
![协调关系的邻接矩阵](./ALC_adjacency_matrix.png)
1. 将上述coupling_send_list_i和coupling_receive_list_i按照模板写入node_i.py对应的地方  
2. 微调ALC_coordination.py文件，运行即可  
## 应用注意事项
